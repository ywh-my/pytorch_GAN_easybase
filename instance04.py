...
'''
GAN + mnist,生成手写数字图片，改良GAN,提高了清晰度但是仍然模式崩溃
'''
...
def train_mnist_GAN():
    ...
    for i in range(epoch):
        for batch in loader:
            ...
            # 使用G 的生成样本训练 D
            D.trainit(G.forward(generate_random((loader.batch_size,100))),torch.zeros(loader.batch_size,1))
            # 训练生成器
            G.trainit(D,generate_random((loader.batch_size,100)),   torch.ones(loader.batch_size,1) )
...
def generate_newimage(G:Generator):
    ...
    for i in range(2):
        for j in range(3):
            output = G.forward(generate_random((1,100)))
            img = output.detach().numpy().reshape(28,28)
